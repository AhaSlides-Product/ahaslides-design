...
export class AhaSwitch extends HTMLElement {
  ...
  attributeChangedCallback() { if (this._track) this._sync(); }

  _build() {
    // Build the subtree ONCE, then let the `checked` host attribute drive :host([checked]) on the
    // persistent .track/.knob. Rebuilding the subtree on toggle destroys the knob and creates a new
    // one already at its end transform, so the CSS transition has no "from" value and snaps — the
    // Switch-click trap. A live node keeps its "from", so the transition actually fires.
    this.shadowRoot.append(document.createRange().createContextualFragment(
      `<style>${STYLE}</style><span class="track" part="track" role="switch" tabindex="0"><span class="knob"></span></span>`));
  ...
  _sync() { this._track.setAttribute('aria-checked', String(this.checked)); }
}

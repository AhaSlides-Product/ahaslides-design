...
export class AhaAlert extends HTMLElement {
  ...
  _render() {
    ...
    // error/warning interrupt assertively (role=alert); info/success are polite (role=status) so they don't barge in
    const role = (type === 'error' || type === 'warning') ? 'alert' : 'status';
    this.shadowRoot.innerHTML = `<style>${STYLE}</style><div class="alert" part="alert" role="${role}">${icon}<div class="body">${title}<div class="msg" part="message"><slot></slot></div></div>${close}</div>`;

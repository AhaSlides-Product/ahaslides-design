...
const STYLE = `
  ...
  :host([icon-only]) .grid{ grid-template-columns:repeat(var(--cols,3), 44px) }
  .card{ box-sizing:border-box; display:flex; flex-direction:column; align-items:center; gap:6px;
    padding:8px; text-align:center; cursor:pointer;
    font-family:var(--aha-font-product,"Plus Jakarta Sans",sans-serif); font-size:13px; line-height:20px;
  ...
  .card:hover{ border-color:var(--aha-color-primary,#6A1EBB); color:var(--aha-color-primary,#6A1EBB) }
  .card:hover aha-icon{ color:var(--aha-color-primary,#6A1EBB) }
  .card[aria-checked="true"]{ border-color:var(--aha-color-primary,#6A1EBB); background:color-mix(in srgb, var(--aha-color-primary,#6A1EBB) 5%, transparent); color:var(--aha-color-primary,#6A1EBB) }
  ...
  :host([disabled]) .card{ cursor:not-allowed; color:var(--aha-text-disabled,#B5B5B5);
    background:var(--aha-gray-25,#F3F3F3); border-color:var(--aha-border-disabled,#EBEBEB) }
  :host([disabled]) .card:hover{ color:var(--aha-text-disabled,#B5B5B5) }
  :host([disabled]) .card aha-icon,
  :host([disabled]) .card:hover aha-icon{ color:var(--aha-text-disabled,#B5B5B5) }
...
export class AhaCardSelect extends HTMLElement {
  ...
  _build() {
    ...
    for (const o of opts) {
      ...
      b.innerHTML = (o.icon ? `<aha-icon name="${o.icon}" size="20" aria-hidden="true"></aha-icon>` : '') +

 ...
 *
 * A page selector for a paged list or table — prev / numbered pages (with ellipsis for long
 * ranges) / next. The current page is brand-filled; hover animates via the shared motion
 * tokens on persistent nodes; prev/next use the DS caret glyph via <aha-icon>. ONE element,
 * shadow-DOM CSS, themed only by --aha-* tokens → byte-identical in React and Vue. Emits a
 * composed `change` event with the new page.
 */

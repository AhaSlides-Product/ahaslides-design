 ...
 *
 *   `help`      — render a built-in `?` help trigger (DS <aha-icon name="system-question-mark">, 16px,
...
const STYLE = `
  ...
  .bubble{ position:absolute; box-sizing:border-box; max-width:200px; padding:6px 10px; white-space:normal;
  ...
  :host([help]) .help:focus-visible{ outline:none; box-shadow:0 0 0 2px var(--aha-color-primary,#6A1EBB) }
...
export class AhaTooltip extends HTMLElement {
  ...
  connectedCallback() {
    ...
    // The settings `?` help pattern sits in a narrow panel with the icon near the left edge, so a
    // top-centred arrowed bubble overflows — default `help` to bottom-start + arrow-off (still
    // overridable: an explicit placement/arrow set by the author wins). (settings-lab default.)
  ...
  _build() {
    ...
    this.shadowRoot.append(document.createRange().createContextualFragment(
      ...
      `<aha-icon name="system-question-mark" size="16" decorative></aha-icon></button>` +

...
const STYLE = `
  ...
  .item{ display:flex; align-items:center; gap:10px; min-height:40px; padding:8px 12px; border:0; text-align:left; width:100%;
    box-sizing:border-box; border-radius:var(--aha-radius-xs,4px); background:transparent; color:var(--aha-text-default,#1A1A1A);
  ...
  /* hover is a NEUTRAL grey wash — not a brand tint (an action menu row isn't a selection) */
  .item:not([disabled]):not(.danger):hover{ background:var(--aha-gray-40,#E3E3E3) }
  ...
  .item.danger:not([disabled]):hover{ background:var(--aha-red-50,#FF4D4F); color:var(--aha-text-inverse,#FFFFFF) }
  ...
  .divider{ height:1px; margin:4px 8px; background:var(--aha-border-disabled,#EBEBEB); border:0 }

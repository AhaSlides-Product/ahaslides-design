...
export class AhaCsat extends HTMLElement {
  ...
  connectedCallback() {
    if (!this.shadowRoot) {
      ...
      this.shadowRoot.innerHTML =
        ...
        `<span class="btns"><button class="btn up" part="button" type="button" aria-label="Yes, helpful"><aha-icon name="system-thumbs-up" size="20" decorative></aha-icon></button>` +
        `<button class="btn down" part="button" type="button" aria-label="No, not helpful"><aha-icon name="system-thumbs-down" size="20" decorative></aha-icon></button></span>`;
  ...
  _update() {
    ...
    this.shadowRoot.querySelector('.up').classList.toggle('selected', value === 'up');
    this.shadowRoot.querySelector('.down').classList.toggle('selected', value === 'down');
  }
}

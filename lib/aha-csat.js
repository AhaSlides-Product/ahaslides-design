 ...
 *
 * Layout: the default is the borderless single row — prompt + two 16px thumb icon-buttons on ONE
 * line, no chrome (the canonical CSAT surface). Add the `card` attribute for the boxed form: 40×40
 * bordered thumb buttons with a semibold prompt. Same thumbs, same logic, same aria-pressed. The
 * legacy `inline` attribute is a no-op — the borderless row it named is now the default.
 */
...
const STYLE = `
  ...
  .btns{ display:inline-flex; gap:4px }
  ...
  /* Card layout — the boxed opt-in form: 40×40 bordered thumb buttons and a semibold prompt. Same
     persistent nodes, so the hover/selected/thanks transitions above still fire (attribute toggle,
     no rebuild). */
  :host([card]){ gap:12px; height:auto }
  :host([card]) .prompt{ font-weight:600 }
  :host([card]) .btns{ gap:8px }
  :host([card]) .btn{ width:40px; height:40px;
    border:1px solid var(--aha-border,#E3E3E3); border-radius:var(--aha-radius-default,8px);
    background:var(--aha-bg-container,#FFFFFF) }
  :host([card]) .btn:hover{ transform:scale(1.08); border-color:var(--aha-border-hover,#D3B4FF); color:var(--aha-color-primary,#6A1EBB) }
  :host([card]) .btn.selected{ border-color:var(--aha-color-primary,#6A1EBB); color:var(--aha-color-primary,#6A1EBB) }
  :host([card]) .thanks{ font-weight:600 }
...
export class AhaCsat extends HTMLElement {
  static get observedAttributes() { return ['prompt', 'value', 'thanks', 'card']; }
  connectedCallback() {
    if (!this.shadowRoot) {
      this.attachShadow({ mode: 'open' });
      this.shadowRoot.innerHTML =
        `<style>${STYLE}</style><span class="prompt" part="prompt"></span>` +
        `<span class="btns"><button class="btn up" part="button" type="button" aria-label="Yes, helpful" aria-pressed="false"><aha-icon name="system-thumbs-up" size="16" decorative></aha-icon></button>` +
        `<button class="btn down" part="button" type="button" aria-label="No, not helpful" aria-pressed="false"><aha-icon name="system-thumbs-down" size="16" decorative></aha-icon></button></span>` +
  ...
  _update() {
    ...
    // <aha-icon> renders at its `size` attribute, not via CSS — so the per-layout thumb size is set here
    const iconSize = this.hasAttribute('card') ? '20' : '16';

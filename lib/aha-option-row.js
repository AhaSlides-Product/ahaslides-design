...
const STYLE = `
  ...
  .row:hover{ border-color:var(--aha-color-primary,#6A1EBB) }
  .row:focus-within{ border-color:var(--aha-color-primary,#6A1EBB); box-shadow:0 0 0 2px var(--aha-border-focus,#D3B4FF) }
  /* correct-answer state — the row turns success-green (scored types); focus keeps a green ring */
  :host([correct]) .row{ border-color:var(--aha-border-success,#16C49A);
    background:color-mix(in srgb, var(--aha-color-success,#16C49A) 8%, transparent) }
  :host([correct]) .row:focus-within{ border-color:var(--aha-color-success,#16C49A);
    box-shadow:0 0 0 2px color-mix(in srgb, var(--aha-color-success,#16C49A) 25%, transparent) }

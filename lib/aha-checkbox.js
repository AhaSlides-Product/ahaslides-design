...
export class AhaCheckbox extends HTMLElement {
  ...
  attributeChangedCallback() { if (this._box) this._sync(); }

  _build() {
    // Build ONCE; checked/indeterminate then drive :host([checked])/:host([indeterminate]) on the
    // persistent .box so its background/border transition fires. Rebuilding the subtree on toggle
    // gives the new .box no "from" state and it snaps (the Switch-click trap). Only the glyph INSIDE
    // the box is content, swapped in place — the box node itself never gets recreated.
    this.shadowRoot.append(document.createRange().createContextualFragment(
      `<style>${STYLE}</style><label><input type="checkbox"/><span class="box"></span><slot></slot></label>`));
  ...
  _sync() {
    ...
    this._box.replaceChildren(document.createRange().createContextualFragment(glyph));
  }
}

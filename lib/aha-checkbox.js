...
const STYLE = `
  ...
  .box{ position:relative; flex:0 0 auto; width:16px; height:16px; box-sizing:border-box;
    border:1px solid var(--aha-border,#E3E3E3); border-radius:4px;
  ...
  :host([disabled]) label:hover .box{ border-color:var(--aha-border,#E3E3E3) }
  ...
  :host([disabled]) .box, :host([disabled][checked]) .box, :host([disabled][indeterminate]) .box{ background:var(--aha-bg-container-disabled,#F1F1F1); border-color:var(--aha-border-disabled,#EBEBEB) }
  :host([disabled]) .bar{ background:var(--aha-text-disabled,#B5B5B5) }
  ...
  :host(:focus-visible) .box{ outline:2px solid var(--aha-color-primary,#6A1EBB); outline-offset:2px }
...
export class AhaCheckbox extends HTMLElement {
  ...
  _build() {
    ...
    // input is aria-hidden: the HOST carries role=checkbox + aria-checked (true/false/mixed), so AT
    // announces the control once. The input stays for form/native change semantics only.
    this.shadowRoot.append(document.createRange().createContextualFragment(
      `<style>${STYLE}</style><label><input type="checkbox" tabindex="-1" aria-hidden="true"/><span class="box">${TICK}<span class="bar"></span></span><slot></slot></label>`));
    this._input = this.shadowRoot.querySelector('input');
    this._input.addEventListener('change', (e) => {
      if (this.disabled) { e.preventDefault(); return; }
      this._toggle(e.target.checked);
    });
    // Keyboard: the host is the focusable checkbox; Space toggles (native <input> is hidden from tab order).
    if (!this.hasAttribute('tabindex')) this.tabIndex = this.disabled ? -1 : 0;
    this.addEventListener('keydown', (e) => {
      if (this.disabled) return;
      if (e.key === ' ' || e.key === 'Spacebar') { e.preventDefault(); this._toggle(!this.checked); }
    });
  }
  _toggle(next) {
    this.checked = next;
  ...
  _sync() {
    ...
    // Expose the state on the host so AT reads it even though the native input is visually hidden:
    // aria-checked is "mixed" for indeterminate, else true/false. role=checkbox + Space (below) complete the contract.

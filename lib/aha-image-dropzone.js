...
const STYLE = `
  ...
  .zone{ box-sizing:border-box; position:relative; width:100%; min-height:120px;
    ...
    border:2px dashed var(--aha-border-strong,#D4D4D4); border-radius:var(--aha-radius-default,8px);
    background:var(--aha-gray-20,#F7F7F7); color:var(--aha-text-tertiary,#8A8A8A);
  ...
  .add aha-icon{ color:var(--aha-text-secondary,#4A4A4A) }
  .hint{ font-size:14px; line-height:21px; color:var(--aha-text-tertiary,#8A8A8A) }
  ...
  .spinner{ width:24px; height:24px; border-radius:var(--aha-radius-pill,999px);
  ...
  .thumb{ position:absolute; inset:0; width:100%; height:100%; object-fit:contain; background:var(--aha-gray-20,#F7F7F7); display:none }
  ...
  .overlay{ position:absolute; inset:0; display:flex; align-items:center; justify-content:center; gap:8px;
    background:var(--aha-ink-a50,rgba(26,26,46,.5)); opacity:0; pointer-events:none;
  ...
  :host([state="filled"]) .zone{ height:135px; min-height:135px; padding:0;
    border:1px solid var(--aha-border,#E3E3E3) }
      ...
      this.shadowRoot.innerHTML = `<style>${STYLE}</style>` +
        `<div class="zone" part="zone">` +
          `<button class="add" part="add" type="button">` +
            `<aha-icon name="system-image-square" size="16" aria-hidden="true"></aha-icon>` +

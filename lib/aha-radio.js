...
export class AhaRadio extends HTMLElement {
  static get observedAttributes() { return ['checked', 'disabled', 'variant', 'size', 'direction']; }
  ...
  connectedCallback() {
    ...
    if (!this._built) {
      ...
      this.shadowRoot.append(document.createRange().createContextualFragment(
        `<style>${STYLE}</style><label><span class="ring"><span class="dot"></span></span><slot></slot></label>`));
      this._built = true;

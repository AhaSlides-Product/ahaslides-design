...
const STYLE = `
  ...
  /* fill via transform:scaleX (compositor-friendly), not width — no layout thrash; the track's
     overflow:hidden + pill radius rounds the visible ends */
  .fill{ height:100%; width:100%; transform:scaleX(0); transform-origin:left center;
    background:var(--aha-color-primary,#6A1EBB);
    transition:transform var(--aha-motion-slow,.3s) var(--aha-ease-out,cubic-bezier(0.215,0.61,0.355,1)), background var(--aha-motion-mid,.2s) var(--aha-ease-in-out,cubic-bezier(0.645,0.045,0.355,1)) }
...
export class AhaProgress extends HTMLElement {
  ...
  _update() {
    ...
    if (type === 'circle') {
      ...
      if (fill) fill.style.transform = 'scaleX(' + (pct / 100) + ')'; // persistent node: only the transform changes

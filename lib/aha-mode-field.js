...
const STYLE = `
  ...
  .head{ display:flex; align-items:center; justify-content:space-between; gap:12px; min-height:32px }
  ...
  /* active = OUTLINE only (border + text primary), no filled tint — the settings-lab radio-outline rule */
  .seg-btn[aria-pressed="true"]{ color:var(--aha-color-primary,#6A1EBB); border-color:var(--aha-color-primary,#6A1EBB); z-index:1 }
...
export class AhaModeField extends HTMLElement {
  ...
  _apply() {
    ...
    for (const child of this.children) {
      ...
      if (cm == null) continue;
      const active = cm === this.mode;
      child.hidden = !active;
      // Data-safety (persistent node + emit-active-only): the bodies stay in the DOM so the active one
      // keeps its state and the transition fires, but a HIDDEN mode's controls must not stay live — a
      // `hidden` input still submits/reads, which would leak a value the user can't see into saved config.
      // So disable a hidden mode's controls, and re-enable ONLY the ones we disabled (marker), so the
      // saved config only ever reflects the active mode.
      const controls = child.querySelectorAll ? child.querySelectorAll('input, select, textarea, button') : [];
      for (const ctl of controls) {
        if (active) { if (ctl.hasAttribute('data-mf-off')) { ctl.disabled = false; ctl.removeAttribute('data-mf-off'); } }
        else if (!ctl.disabled) { ctl.disabled = true; ctl.setAttribute('data-mf-off', ''); }
      }
    }
  }

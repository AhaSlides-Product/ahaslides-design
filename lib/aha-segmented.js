...
const STYLE = `
  ...
  .thumb{ position:absolute; top:2px; bottom:2px; left:2px; width:0;
    background:var(--aha-bg-container,#FFFFFF); border-radius:var(--aha-radius-xs,4px);
    box-shadow:0 1px 2px color-mix(in srgb, var(--aha-gray-100,#1A1A1A) 12%, transparent);
    /* animate only transform (the slide) — width is set imperatively and snaps; never animate a layout
       property (width/height/padding/margin), which thrashes layout (craft-floor: transform/opacity only) */
  ...
  .seg{ position:relative; z-index:1; box-sizing:border-box; border:0; background:none; cursor:pointer;
    ...
    font-family:inherit; font-size:14px; font-weight:400; color:var(--aha-text-secondary,#4A4A4A);

...
// the purple-circle crown badge — the custom mark, never a generic Phosphor crown (PAYWALL-02)
const CROWN = '<span class="crown" role="img" aria-label="Upgrade required"><svg viewBox="0 0 16 16" fill="none"><path d="M3 11.5h10M3.2 5.2l2.4 1.9L8 3.6l2.4 3.5 2.4-1.9-.7 5.1H3.9L3.2 5.2Z" fill="#fff" stroke="#fff" stroke-width="1" stroke-linejoin="round"/></svg></span>'; // ds-lint-allow: hex (white crown on the primary-fill circle)
...
const STYLE = `
  ...
  /* the crown badge — 16px purple circle (PAYWALL-02 / §7) */
  ...
  .crown svg{ width:10px; height:10px; display:block }
...
export class AhaPaywall extends HTMLElement {
  ...
  attributeChangedCallback(name) { if (this.shadowRoot && name !== 'open') this._render(); }

  _render() {
    const label = this._attr('feature-label', 'Pro feature');
    const body = this._attr('body', 'Unlock this feature.');
    const cta = this._attr('cta-label', 'Upgrade');
    const anchor = this.querySelector(':not(script)') ? '<slot></slot>' : CROWN;
    this.shadowRoot.innerHTML = `<style>${STYLE}</style>
      <span class="anchor" part="anchor">${anchor}</span>
      <div class="pop" role="dialog" aria-label="${escAttr(label)}">
        <div class="arrow"></div>
        <div class="phead">${CROWN}<span class="plabel">${esc(label)}</span></div>
        <p class="pbody">${esc(body)}</p>
        <p class="punlock">Unlock with the <b>${esc(PLAN_LABEL[this.plan])} plan</b>.</p>
        <button class="cta" type="button">${esc(cta)}</button>
      </div>`;
    const anchorEl = this.shadowRoot.querySelector('.anchor');
    const trigger = this._attr('trigger', 'click');
    if (trigger === 'hover') {
      this.addEventListener('mouseenter', () => this._openPop());
      this.addEventListener('mouseleave', () => this._close());
    } else {
      anchorEl.addEventListener('click', (e) => { e.stopPropagation(); this.open ? this._close() : this._openPop(); });
    }
    this.shadowRoot.querySelector('.cta').addEventListener('click', (e) => this._upgrade(e));
  }

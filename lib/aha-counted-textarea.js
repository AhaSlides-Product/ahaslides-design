 ...
 *   <aha-counted-textarea maxlength="150" minrows="2" maxrows="5" placeholder="Answer"></aha-counted-textarea>
 *
 * Defaults (the settings-lab defaults): maxlength 150, minrows 2, maxrows 5. Pass `maxlength="none"`
 * to drop the cap and hide the counter.
 *
 ...
 */
const STYLE = `
  :host{ display:inline-flex; width:240px; font-family:var(--aha-font-product,"Plus Jakarta Sans",sans-serif) }
  .wrap{ box-sizing:border-box; position:relative; width:100%; display:flex;
    ...
    border:1px solid var(--aha-border-input,#D3D7E1); border-radius:var(--aha-radius-sm,6px);
  ...
  :host([_counted]) .field{ padding-bottom:22px }
  /* borderless (the compact OptionRow/answer-row case): don't reserve the full gutter — the focus-only
     counter overlays the bottom-right corner, so a single-line answer stays compact (no 2-line-tall row) */
  :host([borderless][_counted]) .field{ padding-bottom:2px }
  ...
  /* textarea exception: hover AND focus stay the LIGHTER border-hover (#D3B4FF), not full primary */
  .wrap:hover{ border-color:var(--aha-border-hover,#D3B4FF) }
  :host([_focused]) .wrap{ border-color:var(--aha-border-hover,#D3B4FF); box-shadow:0 0 0 2px var(--aha-border-focus,#D3B4FF) }
...
export class AhaCountedTextarea extends HTMLElement {
  ...
  _render() {
    this.shadowRoot.innerHTML = `<style>${STYLE}</style>` +
      `<div class="wrap" part="wrap">` +
        `<textarea class="field" part="field" rows="1"></textarea>` +
  ...
  _autogrow() {
    ...
    const max = Math.max(min, parseInt(this.getAttribute('maxrows') || '5', 10));
  ...
  // maxlength defaults to 150 (the settings-lab default); `maxlength="none"` drops the cap + counter.
  _maxLength() {

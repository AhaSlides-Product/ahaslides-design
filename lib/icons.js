/**
 * @ahaslides/design/icons — the shared, importable icon system.
 *
 *   import '@ahaslides/design/icons';                 // registers <aha-icon> (browser)
 *   import { icons, ICON_NAMES } from '@ahaslides/design/icons';   // the catalogue (any runtime)
 *
...
function draw(el) {
  ...
  if (!ic) {
    el.shadowRoot.innerHTML = `<span title="unknown icon: ${name}" style="display:inline-block;box-sizing:border-box;width:${size}px;height:${size}px;border:1px dashed #F5222D;border-radius:3px"></span>`;
    return;
  }
...
// Auto-register on import in a browser, so `import '@ahaslides/design/icons'` is all a consumer needs.

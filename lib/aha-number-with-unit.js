...
export class AhaNumberWithUnit extends HTMLElement {
  ...
  _render() {
    ...
    f.addEventListener('input', () => {
      // hard digit cap — a keystroke past maxDigits is dropped (SETTINGS-45)
      let digits = f.value.replace(/[^\d]/g, '').slice(0, this._maxDigits);
      if (f.value !== digits) f.value = digits;

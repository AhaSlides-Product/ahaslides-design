 ...
 *   <aha-number-with-unit value="20" unit="sec" min="5" max="120" maxdigits="4"></aha-number-with-unit>
 *
 * The settings field for a number that carries a UNIT — a duration, points, a per-item count
 * (SETTINGS-45). It is a fixed-width digit input with the unit written as its conventional SHORT label
 * INSIDE the same field (a seconds unit is always `sec`, never `s` or `seconds` — SETTINGS-45/46) — a
 * bordered box beside the input would imply the unit is selectable. A hover-revealed ▲/▼ tertiary
...
const STYLE = `
  :host{ display:inline-flex; flex-direction:column; gap:4px; align-items:flex-start }
  .wrap{ box-sizing:border-box; position:relative; width:auto; display:inline-flex; align-items:center; gap:4px;
  ...
  /* fixed-width digit chip — 4ch tabular-nums, no flex-grow, so the trio packs left (matches settings-lab) */
  .field{ box-sizing:content-box; flex:0 0 auto; width:4ch; height:100%; padding:0; margin:0;
    font-family:var(--aha-font-product,"Plus Jakarta Sans",sans-serif); font-size:15px; line-height:21px;
  ...
  .unit{ flex:0 0 auto; font-family:var(--aha-font-product,"Plus Jakarta Sans",sans-serif);
    font-size:14px; line-height:21px; color:var(--aha-text-placeholder,#999999); user-select:none; white-space:nowrap }

  /* the ▲/▼ stepper — a persistent column revealed on hover/focus (opacity, no remount) */
  /* asymmetric gaps: field→stepper 6px (wrap gap 4 + this 2), stepper→unit 4px (wrap gap) */
  .stepper{ flex:0 0 auto; margin-left:2px; display:inline-flex; flex-direction:column; gap:0; opacity:0;
  ...
  .wrap:hover{ border-color:var(--aha-color-primary,#6A1EBB) }
  :host([_focused]) .wrap{ border-color:var(--aha-color-primary,#6A1EBB); box-shadow:0 0 0 2px var(--aha-border-focus,#D3B4FF) }

 ...
 *
 * ORIENTATION — `tab-position="left"` turns the horizontal bar into a vertical left rail: the tabs
 * stack down the side and the panel sits to the right. The active indicator moves from a 2px bottom
 * underline to a 2px side border on the active (right) edge for line/primary; card keeps its pill.
 * The bar's bottom rule becomes the rail's right-edge rule. Roving arrow-key nav switches to
 * ArrowUp/ArrowDown (Home/End unchanged). Default stays top/horizontal.
 *
 * TWO ways to feed it:
...
const STYLE = `
  ...
  /* ── vertical / left-rail orientation ─────────────────────────────────────
     The host becomes a two-column flex: a vertical tab rail + the panel to its
     right. The bar's bottom rule turns into a right-edge rule; the 2px active
     underline turns into a 2px active side border on the rail's right edge.
     Only orientation flips here — the SAME persistent .tab nodes toggle .on, so
     the border-color transition still fires (no subtree rebuild on switch). */
  :host([tab-position="left"]){ display:flex; align-items:flex-start; gap:24px }
  :host([tab-position="left"]) .bar{ flex-direction:column; gap:8px; align-items:stretch;
    border-bottom:0; border-right:1px solid var(--aha-border,#E3E3E3); flex:0 0 auto }
  :host([tab-position="left"]) .tab{ justify-content:flex-start; text-align:left;
    padding:8px 16px 8px 0; margin-bottom:0; margin-right:-1px;
    border-bottom:0; border-right:2px solid transparent }
  :host([tab-position="left"]) .tab.on{ border-right-color:var(--aha-color-primary,#6A1EBB) }
  :host([tab-position="left"]) .panels{ flex:1 1 auto; padding-top:0 }

  /* vertical + card: the pill still fills; no right rule needed on the active pill */
  :host([tab-position="left"][type="card"]) .bar{ border-right:0; gap:8px }
  :host([tab-position="left"][type="card"]) .tab{ padding:8px 12px; margin-right:0;
    border-right:2px solid transparent }
  :host([tab-position="left"][type="card"]) .tab.on{ border-right-color:transparent }

  :host([tab-position="left"][size="small"]) .bar{ gap:6px }
  :host([tab-position="left"][size="small"]) .tab{ padding:6px 14px 6px 0 }
  :host([tab-position="left"][size="small"][type="card"]) .tab{ padding:6px 10px }
...
export class AhaTabs extends HTMLElement {
  static get observedAttributes() { return ['value', 'items', 'type', 'size', 'tab-position']; }
  attributeChangedCallback(name) {
    if (!this.shadowRoot) return;
    if (name === 'value') this._select(this.value, false);
    else this._render();   // items/type/size/tab-position change → full rebuild (not a per-state toggle, so no dead transition)
  }

  get vertical() { return this.getAttribute('tab-position') === 'left'; }
  ...
  _render() {
    ...
    this.shadowRoot.innerHTML = `<style>${STYLE}</style><div class="bar" part="bar" role="tablist" aria-orientation="${this.vertical ? 'vertical' : 'horizontal'}">${tabs}</div>` +
  ...
  _onKey(e) {
    ...
    if (!count) return;
    // Roving nav follows the orientation: vertical rail → Up/Down; horizontal bar → Left/Right.
    // Home/End work in both. Arrow keys off-axis are ignored so they don't steal the caret.
    const fwd = this.vertical ? 'ArrowDown' : 'ArrowRight';
    const back = this.vertical ? 'ArrowUp' : 'ArrowLeft';
    let next = this.value;
    switch (e.key) {
      case fwd: next = this._step(this.value, 1); break;
      case back: next = this._step(this.value, -1); break;

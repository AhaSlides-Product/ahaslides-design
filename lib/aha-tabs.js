...
export class AhaTabs extends HTMLElement {
  ...
  connectedCallback() {
    ...
    const tabs = this._panels.map((p, i) => `<button class="tab" type="button" part="tab" role="tab" data-i="${i}">${p.getAttribute('data-tab')}</button>`).join('');
  ...
  _select(index, emit) {
    ...
    this._tabs.forEach((t, i) => { t.classList.toggle('on', i === index); t.setAttribute('aria-selected', i === index); });

 ...
 *   </aha-result>
 *
 * A full-block result state — success / error / info / warning / 404 — with a DS icon, a title,
 * a subtitle and a slot for follow-up actions. ONE element, shadow-DOM CSS, themed only by
 * --aha-* tokens. The status glyph is the shared <aha-icon> BY NAME, never an inline <svg>.
 */
...
const STYLE = `
  ...
  :host([status="error"]) .icon{ color:var(--aha-color-error,#F5222D) }
  :host([status="warning"]) .icon{ color:var(--aha-color-warning,#FF7747) }
...
export class AhaResult extends HTMLElement {
  static get observedAttributes() { return ['status', 'result-title', 'subtitle']; }
  connectedCallback() {
    if (!this.shadowRoot) {
  ...
  _update() {
    ...
    this.shadowRoot.querySelector('aha-icon').setAttribute('name', GLYPH[status] || GLYPH.info);

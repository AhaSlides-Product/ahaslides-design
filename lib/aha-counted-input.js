 ...
 *
 * maxlength defaults to 100 (the settings-lab default); pass `maxlength="none"` to drop the cap and
 * hide the counter entirely.
 *
...
const STYLE = `
  ...
  :host([size="sm"]) .wrap, :host([size="small"]) .wrap{ height:32px; padding:0 8px }
  :host([size="lg"]) .wrap, :host([size="large"]) .wrap{ height:48px }
...
export class AhaCountedInput extends HTMLElement {
  ...
  // maxlength defaults to 100 (the settings-lab default); `maxlength="none"` drops the cap + counter.
  _maxLength() {

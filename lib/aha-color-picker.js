...
export class AhaColorPicker extends HTMLElement {
  ...
  // `value` is observed so a controlled (framework-bound) or external attribute change re-syncs the
  // selected swatch + its aria-pressed — not just the property setter path.
  static get observedAttributes() { return ['value']; }
  attributeChangedCallback() { if (this.shadowRoot) this._paint(); }

  connectedCallback() {
    if (!this.shadowRoot) this.attachShadow({ mode: 'open' });
    const cells = this.swatches.map(c => `<button class="swatch" type="button" part="swatch" data-c="${c}" aria-label="${c}" aria-pressed="false" style="background:${c}"></button>`).join('');

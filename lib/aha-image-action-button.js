...
const STYLE = `
  ...
  .trigger{ box-sizing:border-box; display:inline-flex; align-items:center; justify-content:center;
    width:36px; height:36px; padding:0; overflow:hidden; cursor:pointer; font-size:18px;
  ...
  .trigger:hover{ border-color:var(--aha-purple-50,#8644D4); color:var(--aha-purple-50,#8644D4);
    background:var(--aha-bg-accent,#F9F5FF) }
  ...
  .thumb{ width:100%; height:100%; object-fit:cover; border-radius:var(--aha-radius-xs,4px); display:none }
  .overlay{ position:absolute; inset:0; display:flex; align-items:center; justify-content:center;
    color:var(--aha-text-inverse,#fff); background:var(--aha-ink-a50,rgba(26,26,46,.5));
    border-radius:var(--aha-radius-xs,4px); opacity:0; pointer-events:none;
    transition:opacity var(--aha-motion-fast,.1s) var(--aha-ease-out,cubic-bezier(0.215,0.61,0.355,1)) }
  :host([state="filled"]) .trigger:hover, :host([state="filled"]) .trigger:focus-visible{ border-color:var(--aha-color-primary,#6A1EBB) }
...
export class AhaImageActionButton extends HTMLElement {
    ...
    if (!this._built) {
      this.shadowRoot.innerHTML = `<style>${STYLE}</style>` +
        `<button class="trigger" part="trigger" type="button">` +
          `<aha-icon class="add" name="system-image-square" size="18" aria-hidden="true"></aha-icon>` +
  ...
  _sync() {
    ...
    t.setAttribute('aria-label', this.state === 'empty' ? (this.getAttribute('label') || 'Add image') : 'Edit image');

...
const STYLE = `
  ...
  /* progress bar — a persistent track; the fill scales on the compositor (transform, not width) */
  .bar{ height:4px; border-radius:var(--aha-radius-pill,999px); background:var(--aha-split,#F1F1F1); overflow:hidden }
  .bar > i{ display:block; height:100%; width:100%; border-radius:inherit; background:var(--aha-color-primary,#6A1EBB);
    transform:scaleX(0); transform-origin:left center;
    transition:transform var(--aha-motion-slow,.3s) var(--aha-ease-in-out,cubic-bezier(0.645,0.045,0.355,1)) }
...
export class AhaUploader extends HTMLElement {
  ...
  _rowHtml(it, i) {
    ...
    const progress = status === 'uploading'
      ? `<div class="bar" role="progressbar" aria-valuenow="${pct}" aria-valuemin="0" aria-valuemax="100"><i style="transform:scaleX(${pct / 100})"></i></div>` : '';

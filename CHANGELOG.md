...
## 0.37.0 — 2026-09-18
### Changed
- **Settings controls aligned to the settings-lab reference (17 controls).** Reconciled the whole settings component set against the product's canonical settings surface — the DS keeps its architecture (Shadow-DOM custom elements, `--aha-*` tokens, gates) and adopts settings-lab's function/visual. **Input family** (`aha-counted-input`/`aha-counted-textarea`/`aha-select`/`aha-number-with-unit`): rest border `--aha-border-input` #D3D7E1, placeholder `--aha-text-placeholder` #999999, h40 (small 32 / large 48), single-line hover → full-primary #6A1EBB, solid 2px #D3B4FF focus ring; textarea keeps the lighter #D3B4FF hover at radius 6 / maxRows 5. `aha-number-with-unit`: stepper now sits **left of the unit** (a child-order render-gate guards it), compact 4ch chip, unit stays `sec`. `aha-option-row`: success-green correct-answer row state. `aha-checkbox` 16×16 / #E3E3E3 border. `aha-segmented` label weight 400 / thumb r4 (and the thumb no longer animates a layout property). `aha-card-select` card + `color-mix` selected fill. `aha-numbered-item` r12 card + muted-grey chip. `aha-mode-field` outline-only active + hidden-mode controls disabled (a hidden mode can't leak into saved config). `aha-image-dropzone` 2px-dashed/135px frame, `aha-image-action-button` 36×36. `aha-dropdown` 40px items. `aha-tooltip` help defaults to `bottom-start` + no arrow. `aha-info-box` error tone → Radical Pink (`pink-20`/`pink-60`), warmer `bgWarningSubtle` warning fill, padding 12/16. New tokens: `--aha-border-input`, `--aha-text-placeholder`, `--aha-bg-warning-subtle`. (#103)
...
- **Design-language judge** — `DESIGN-LANGUAGE-JUDGE.md`, the page/screen-level verdict counterpart to the charter (companion to the per-component `*-judge` skills): it routes a screen to its world, then emits a binary PASS/FAIL per principle with ordered fixes, to close a build→judge→fix loop. (#84)
- **Screen-lint** — a new mechanical gate: `node screen-lint.mjs --surface=product|canvas <files>` (`./screen-lint` export, `npm run lint:screen`). It hard-fails the zero-interpretation subset of the design language on a consumer screen — raw hex / off-scale radius / gradient fills / icon-only-without-an-accessible-name (Product UI); hardcoded colour / sub-16px or viewport-unit fonts (Canvas) — with a `ds-lint-allow` escape hatch, and is now part of `npm run check`. Statically-undecidable rules (token-layer contrast, copy, right-instrument) stay the judge's job. (#84)

### Fixed
- **Badge snippet** — the paste-and-run example set a raw `background:#fff` on its demo container; now bound to `var(--aha-bg-container,#fff)` (the first defect caught by the new screen-lint gate). (#84)

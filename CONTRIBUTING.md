...
- **Hosting:** the snippets import the element ESM publicly from **jsDelivr `/gh/`** pinned to a tag — `https://cdn.jsdelivr.net/gh/ahaslides-product/ahaslides-design@<tag>/lib/…`. This serves the repo's committed `lib/*.js` directly (no npm needed; the published package is GitHub-Packages-only). **On each release, bump the tag** (e.g. `v0.1.0` → `v0.1.1`) and re-pin the snippets so paste-and-run loads the new element code. (Public npm / a branded CDN remain optional later polish.)

...
`qa.mjs` proves your component *renders and animates*; it proves **nothing** about roles, keyboard, or aria — so a component can look perfect and be unusable by keyboard or a screen reader (a whole batch shipped that way, PRO38-8). `standards.mjs` now scans your `lib/` element source for the three ways that goes wrong. **All are a HARD FAIL for a new component**; the escape hatch is `ds-lint-allow: a11y (why)` on the line, and `A11Y_DEBT` in `standards.mjs` (mirrors `MOTION_DEBT`) grandfathers real pre-gate debt as WARN — it's empty today, so the gate is fully hard.

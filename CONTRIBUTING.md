...
- **Leaf element — motion must be token-bound, and it must actually fire.** If the element has interactive states (`:hover`, `:focus`, `:checked`, `[open]`, a close affordance…), those states MUST **animate, not snap**, using the shared motion tokens — `--aha-motion-*` durations + `--aha-ease-*` curves (AntD's motion; e.g. `transition: transform var(--aha-motion-mid) var(--aha-ease-in-out-circ)`). The gate flags **five** ways motion goes wrong:
  ...
  5. **Bounce/elastic easing** — a `cubic-bezier` whose control-point Y leaves `0–1` overshoots (a "back"/elastic curve). It reads dated and isn't how AntD (or a real object) decelerates — use an exponential ease-out (`--aha-ease-out` / `--aha-ease-in-out`). The gate flags the overshoot.
  Escape hatch `ds-lint-allow: motion (why)`. A purely static marker with no interactive state (e.g. Badge) needs no transition and isn't flagged. **All five are a HARD FAIL for a new component** — it can't merge with a motion defect. The only grace is `MOTION_DEBT` in `standards.mjs`: a small, explicit, greppable allow-list of components that already shipped a defect before this gate existed (currently `aha-switch`/`aha-checkbox`/`aha-tooltip` = dead, `aha-paywall` = snap), which stay WARN until restructured. Remove each entry as it's fixed; when `MOTION_DEBT` is empty the gate is fully hard, no exceptions.
...
### 4d. The accessibility standard — it must be OPERABLE, not just drawn (enforced)
`qa.mjs` proves your component *renders and animates*; it proves **nothing** about roles, keyboard, or aria — so a component can look perfect and be unusable by keyboard or a screen reader (a whole batch shipped that way, PRO38-8). `standards.mjs` now scans your `lib/` element source for the four ways that goes wrong. **All are a HARD FAIL for a new component**; the escape hatch is `ds-lint-allow: a11y (why)` on the line, and `A11Y_DEBT` in `standards.mjs` (mirrors `MOTION_DEBT`) grandfathers real pre-gate debt as WARN — it's empty today, so the gate is fully hard.
  1. **Roving role without arrow keys** — if you declare a roving-widget `role` (`radio`/`radiogroup`, `tab`/`tablist`, `menu`/`menuitem`, `option`/`listbox`, `treeitem`, `combobox`, …), the arrow keys **are** the interaction model: one tab-stop, roving focus, `ArrowUp/Down/Left/Right` to move. A click handler alone makes the announced role a lie. Implement roving-`tabindex` + arrow-key handling, or **don't declare a role whose contract you don't meet** (a plain group of `<button>`s with `aria-pressed` is often the honest choice).
  2. **Leaked global listener** — a listener bound to `document`/`window` in `connectedCallback` **must** be removed in `disconnectedCallback` (same event). An anonymous handler can't be removed at all — store it (`this._onKey = …`) and remove it. Otherwise it leaks on every mount/unmount and pins the element.
  3. **Dynamic aria without `observedAttributes`** — if you set an `aria-*` **state** (`aria-expanded`/`checked`/`selected`/`pressed`/`current`/`valuenow`) imperatively, that state is dynamic, so its host attribute is too. Declare `observedAttributes` + an `attributeChangedCallback` that **re-syncs the aria**. Without it, a framework binding the attribute (or a consumer `setAttribute`) flips your `:host([state])` visuals while the aria value stays frozen at its mount value — the screen reader reads the wrong state (the controlled-`collapse` case).
  4. **Silent toggle button** — a `<button>` that toggles a selection class (`selected`/`active`/`pressed`/`on`) on state but exposes no `aria-pressed`/`aria-checked` reads as a plain button to a screen reader — the selection is invisible. Sync `aria-pressed` (or `aria-checked`) to the state in the same place you toggle the class (the CSAT thumbs / colour-picker swatch case). A button that carries a roving/selectable role (`radio`/`tab`/`menuitem`/`option`) is exempt — it already syncs its own aria.

  Two more, not statically catchable — **you own them, the reviewer checks them**: (a) declare the **right** role in the first place — a progress bar needs `role="progressbar"` + `aria-valuenow/min/max`, a spinner `role="status"`; a control with no role is invisible to AT. (b) A **themed imperative overlay** in a snippet — antd's `message`/`notification`/`Modal` have a *static* form (`message.success()`, `notification.open()`, `Modal.confirm()`) that renders **outside** `ConfigProvider`, so it ships **un-themed**. The gate flags the static call in a snippet: use the **hook** form (`useMessage()` / `useNotification()` / `useModal()` + `contextHolder`) so the overlay consumes the DS theme in **every** framework snippet, not just the conformance harness.
